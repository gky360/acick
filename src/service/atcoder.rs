...
impl<'a, 'b> AtcoderService<'a, 'b> {
    pub fn new(client: Client, conf: &'a Config, ctx: &'a mut Context<'b>) -> Self {
        Self { client, conf, ctx }
    }
}

impl Serve for AtcoderService<'_, '_> {
    fn login(&mut self, user: String, pass: String) -> Result<LoginOutcome> {
        let Self { client, conf, ctx } = self;
        let login_page = LoginPageBuilder::new(conf).build(client, ctx)?;
        ...
        let settings_page = SettingsPageBuilder::new(conf).build(client, ctx)?;
    ...
    fn fetch(&mut self, problem_id: &Option<ProblemId>) -> Result<Contest> {
        ...
        let tasks_page = TasksPageBuilder::new(conf).build(client, ctx)?;
        ...
        let tasks_print_page = TasksPrintPageBuilder::new(conf).build(client, ctx)?;
